...
# Export the primary Figure 2; companion and diagnostic graphics are created
...
# Reproduce the primary figure's y-axis calculation as a tabular
...
contract_text <- c(
  ...
  "- adjusted partial-residual bin means are omitted from the uncluttered primary export and retained in a diagnostic companion export and Source Data;",
...
writeLines(legend_text, file.path(dirs$report, "Figure2_legend.txt"))

diagnostic_companion_legend <- paste0(
  legend_text,
  " In the diagnostic companion, open circles are geometric means of adjusted partial residuals in ten equal-frequency exposure bins; for observation i and exposure term j, the partial residual was defined on the model's Gaussian identity scale as the fitted contribution of term j plus the full-model response residual. These descriptive summaries are not independent observations and their dispersion should not be compared with the shaded model-parameter confidence bands, which are not outcome prediction intervals."
)
writeLines(
  diagnostic_companion_legend,
  file.path(dirs$report, "Figure2_adjusted_partial_residual_companion_legend.txt")
)

supplementary_influence_legend <- paste0(
  "Observation-level leverage and influence diagnostics for the six frozen Model C fits. ",
...
supplementary_sensitivity_legend <- paste0(
  "Influence-excluded sensitivity of climate-AMR exposure-response shapes. ",
...
terminology_ledger <- c(
  ...
  "- Rap/Tre: earlier aliases that are not executed or reported as geometric classes in this analysis",
...
audit_text <- c(
  ...
  "## Threshold overlays",
  "",
  "The primary export overlays every objectively detected internal extremum using the same red long-dashed line and reports its term-centred pointwise OR (95% interval). Filled red markers indicate extrema meeting the 900/1,000 bootstrap-stability criterion and open red markers indicate that the criterion was not met; the numerical support is not printed in the main panel and every candidate remains visible. Pointwise OR significance is not a detection or retention criterion. Exact stability, conditional location intervals and local observation and country support remain in Source Data. No hard-coded target is used.",
  ...
  "",
  "The uncluttered primary export omits adjusted partial-residual circles. The diagnostic companion retains them as descriptive adjusted response summaries; they are not validation confidence intervals and are not expected to lie inside the shaded term-wise confidence band. Parametric-bootstrap support is conditional on the observed data configuration and is not a test of resistance to influential observations; the separately reported influence-exclusion refits address that distinct question. Clustered uncertainty is evaluated in the separate country-cluster sensitivity analysis."
)
...
threshold_axis_audit <- c(
  ...
  "",
  "## Implemented turning-point procedure",
  "",
  "The implemented procedure searches only the phenotype–exposure-specific P2.5–P97.5 domain on the model-input scale, evaluates each fitted GAM smooth directly and defines internal extrema by first-derivative sign changes. It applies no phenotype-specific target priorities or OR-significance filters and uses 1,000 Gaussian response simulations followed by REML bam refits. Bootstrap stability is indicated when an extremum is matched in at least 900 of 1,000 attempted refits. Chg and Stb are calculated separately by the phenotype-agnostic supplementary feature algorithm and do not select main-figure turning points.",
  "",
  "## Main-figure display decision",
  "",
  "The primary export shows all internal extrema detected under the reported objective algorithm. Each label gives the term-centred OR and pointwise 95% interval. All extrema use the same red long-dashed line; marker fill alone indicates whether the >=90% stability criterion was met, and exact support is retained in Source Data. No candidate is removed for low support. Adjusted partial-residual bin means are removed from the primary figure to reduce clutter but retained in the diagnostic companion, Source Data workbook and observation-level diagnostic outputs.",
  "",
...
s2_method_path <- file.path(
  dirs$report, "Supplementary_Method_S2_executed_algorithm.txt"
)
writeLines(
  c(
...
writeLines(
  ...
  file.path(dirs$report, "Main_Methods_GAMM_and_turning_points.txt")
)
readme_text <- c(
  "# Figure 2 outputs",
  "",
  ...
  "",
  "This file shows every objectively detected internal turning point using a uniform red long-dashed line and reports the physical-equivalent location to two decimal places together with the term-centred pointwise OR (95% interval). Filled versus open red markers indicate whether the 900/1,000 stability criterion was met; numerical bootstrap support is retained in Source Data and no secondary display cutoff is imposed. Every curve panel has exactly five y-axis ticks. Panels with EDF<=1.05 use a common 0.5–1.5 OR scale, with separate annotations for terms shrunk towards zero, approximately linear terms and low-complexity curved fits.",
  "",
  ...
  "",
  "## Diagnostic companion file",
  "",
  "`02_figures/Figure2_turning_points_termcentred_OR_95CI_with_support_symbols.pdf`",
  "",
  "This diagnostic companion retains turning-point labels and adjusted partial-residual bin means. The supplementary feature catalogue implements Chg and Stb using phenotype-agnostic rules; a T prefix records pointwise P>=0.10.",
  "",
  "## Model invariance",
  "",
  "The six frozen primary Model C objects are not refitted by this figure workflow, and all 24 primary curves and confidence limits reproduce the validated source data to numerical precision. Six separately identified influence-excluded models are diagnostic sensitivity fits only and do not feed threshold detection or Results 2–3.",
  "",
...
cat("Primary threshold-preserving Figure 2:", paste0(stem_combined_thresholds_no_partial_bins, ".pdf"), "\n")
cat("Diagnostic companion Figure 2:", paste0(stem_combined_thresholds, ".pdf"), "\n")
